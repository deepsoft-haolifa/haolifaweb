...
<div class="select-box" :class="{'select-viewTop': viewTop,'select-active':show,'select-disabled':disabled}">
  ...
  <transition name="slide-y">
    <ul class="select-option scroll-y" v-if="show">
      <li v-for="(item,i) in (filterable ? filterList : list)" :class="{ 'active': item[optionText] === model, 'disabled': item[optionDisabled] }" :value="item[optionValue]" @click.stop.prevent="select(item)" :key="i">{{item[optionText]}}</li>
    </ul>
  </transition>
</div>
</template>
...
.select-box{ display: block;position:relative; color:rgba(0,0,0,.87);font-size:14px;
  ...
  .select-option li.disabled { color: #c0c4cc;cursor: not-allowed; }
  .select-option li.disabled:hover { background: inherit; }
}
</style>
