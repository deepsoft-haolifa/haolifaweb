...
<div class="page-room-add abs scroll-y">
  <div class="form-content">
    ...
    <div class="flex-v-center" v-for="(item,i) in form.unqualifiedList" :key="item.reason">
      <select-box v-model="item.reason" class="flex-item mr-10" :list="reasonList" label="不合格原因">
      </select-box>
...
export default {
  ...
  computed: {
    canSubmit () {
      const { form } = this
      return form.inspectNo && form.orderNo && form.testingNumber && form.qualifiedNumber && form.unqualifiedNumber && form.reinspectNumber
    }
  },
  created () {
      this.getReasonList();
  ...
  methods: {
    getReasonList(){
        this.$http.get(`/haolifa/pressure-inspect-res/pressure_reasonList`).then(res => {
    ...
    addReason(){
        this.form.unqualifiedList.push({reason:"",unqualifiedNumber:""})
    },
    getInfo(inspectNo){
    ...
    submit () {
      ...
      this.loading = true
      const method = 'post'
      this.$http[method](`/haolifa/pressure-inspect-res/${form.id ? 'update' : 'save'}`, form).then(res => {
        this.loading = false
        this.$router.replace('/stresstest')
      }).catch(e => {
        this.loading = false
        this.$toast(e.msg || e.message)
      })
