...
export default {
  ...
  created () {
    ...
    if (id !== undefined && this.$route.name === 'supplierequipment-edit') this.getInfo(id)
  },
  methods: {
    getInfo (id) {
      this.$http.get(`/haolifa/equipment/getInfo/${id}`).then(res => {
    ...
    submit () {
      const { form } = this
      this.loading = true
      this.$http.post(`/haolifa/equipment/supplier-${form.id ? 'update' : 'save'}`, form).then(res => {
        this.loading = false
