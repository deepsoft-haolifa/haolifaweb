...
export default {
  ...
  computed: {
    canSubmit () {
      const { form } = this
      return !!form.name && !!form.productNo && form.productMaterialList.every(v => {
        return v.materialClassifyId && v.materialCount && v.materialGraphNo
      })
    }
  },
  created () {
  ...
  methods: {
    addProductMaterialList () {
      this.form.productMaterialList.push({id: Math.random(), materialClassifyId: '', materialCount: '', materialGraphNo: '', replaceMaterialGraphNo: ''})
    },
    ...
    getTypeList () {
      this.$http.get(`/haolifa/material/classify/list`).then(res => {
        this.typeList = res.filter(item => !item.isDelete).map(item => {
          return { value: item.id, text: item.classifyName, list: [] }
        })
        console.dir(res)
      })
    },
    getInfo (id) {
      this.$http.get(`/haolifa/product/getInfo/${id}`).then(res => {
        ...
        this.changeFlag = this.form.productMaterialList.map(v => false)
