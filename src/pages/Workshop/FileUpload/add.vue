...
export default {
  ...
  data () {
    return {
      typeList: [{text: '零件图纸', value: 1}, {text: '其它', value: 2}],
      fileList:[],
      loading: false,
      form: {
        id: '',
  ...
  computed: {
    canSubmit () {
      const { form } = this
      return form.fileName && form.type && form.fileUrl && form.remark && form.fileBase64
    }
  },
  created () {
    let { id } = this.$route.query
    if (id !== undefined && this.$route.name === 'fileupload-edit') this.getInfo(id)
  },
  methods: {
    getInfo (id) {
      this.$http.get(`/haolifa/file/info/${id}`).then(res => {
    ...
    submit () {
      ...
      this.$http[method](`/haolifa/file/${form.id ? 'update' : 'save'}`, form).then(res => {
        this.loading = false
