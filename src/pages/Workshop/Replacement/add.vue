...
export default {
  ...
  created () {
    ...
    if (id !== undefined && this.$route.name === 'replacement-edit') this.getInfo(id)
  },
  methods: {
    getInfo (id) {
      this.$http.get(`/haolifa/replace-material/info/${id}`).then(res => {
    ...
    submit () {
      ...
      this.$http[method](`/haolifa/replace-material/${form.id ? 'update' : 'save'}`, form).then(res => {
        this.loading = false
