        ...
        <div class="flex-item scroll-y">
            <data-list ref="list" :page-size="10" :param="filter" url="/haolifa/entrust/list" method="post">
                ...
                <template slot="item" slot-scope="{ item, index }">
                    ...
                    <td class="t-right">
                        <a href="javascript:;" v-if="item.workshopType == '2' && item.status == 2"
                           style="margin-right: 3px" class="blue" @click="machining(item.entrustNo, 4)">处理完成</a>
                        <a href="javascript:;" v-if="item.status == 0" style="margin-right: 3px" class="blue"
                           @click="remove(item)">删除</a>
                        <a href="javascript:;" v-if="item.status == 0" style="margin-right: 3px" class="blue"
                           @click="initApprove(item.entrustNo, 1)">发起审批</a>
                    </td>
                </template>
            </data-list>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            initApprove(entrustNo,status) {
                this.$http.get(`/haolifa/entrust/updateStatus/${entrustNo}/${status}`).then(res => {
                    this.$refs.list.update();
                }).catch(e => {
                    this.$toast(e.msg || e.message)
                })
